...
Public Class ucBrxExport
    ...
    Public conditionISDU As String = ""
    ...
    Private Sub ComboBox1_SelectedValueChanged(sender As Object, e As EventArgs) Handles cbSelectedOption.SelectedValueChanged
        ...
        Else
            ...
            If Not IsDBNull(CType(cbSelectedOption.SelectedItem, DataRowView).Item("conditionValue")) Then
                ...
                conditionISDU = CType(cbSelectedOption.SelectedItem, DataRowView).Item("conditionISDU")
            Else
                conditionVal = ""
                conditionDesc = ""
                conditionISDU = "0"
...
<li>If you used indirect mode (as descibed above):
    ...
    Private Sub ExportISDUWriteToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ExportISDUWriteToolStripMenuItem.Click
        Dim lstRungCommands As New List(Of String)
        Dim lstUDTconfig As New List(Of String)
        Dim lstMEMconfig As New List(Of String)


        lstRungCommands.Add("Str ISDU_RW_BUF.Write")
        lstRungCommands.Add("COPY 0x1 ""4 0x0 " & conditionISDU & " ISDU_RW_BUF.Index 1 0x0 0 ISDU_RW_BUF.SubIndex 1 0x0 " & conditionVal & " ISDU_RW_BUF.Value 1""")
        lstRungCommands.Add("STRPRINT ISDU_RW_BUF.ISDUReq 0x0 ""Raw(ISDU_RW_BUF.Index,2) Raw(ISDU_RW_BUF.SubIndex,1) Raw(ISDU_RW_BUF.Value,1)""")
        Dim explMesPar As explMsgParam
        explMesPar.Class = "0x83" : explMesPar.Instance = "0x3" : explMesPar.attribute = "DST511"
        explMesPar.genericService = "0x25F" : explMesPar.specificService = "0x4C"
        explMesPar.requestStringStructure = "ISDU_RW_BUF.ISDUReq" : explMesPar.resultStringStructure = "ISDU_RW_BUF.ISDURes"
        explMesPar.resultGeneralStatusCode = "ISDU_RW_BUF.Stat" : explMesPar.resultErrorBit = "ISDU_RW_BUF.Error" : explMesPar.resultSuccessBit = "ISDU_RW_BUF.Success"
        '        lstRungCommands.Add("EIPMSG @IntEIPClient 0 0 0x83 0x2 DST511 0x25F 0x4C IOL_X1_rgb.ISDUReq DST511 DST511 IOL_X1_rgb.ISDURes DST511 DST511 DST511 IOL_X1_rgb.ISDU.Stat SL0 DST511 DST511 DST511 0 0x0 IOL_X1_rgb.ISDU.Success IOL_X1_rgb.ISDU.Error DST511 DST511")
        lstRungCommands.Add(String.Format("EIPMSG @IntEIPClient 0 0 {0} {1} {2} {3} {4} {5} DST511 DST511 {6} DST511 DST511 DST511 {7} SL0 DST511 DST511 DST511 0 0x0 {8} {9} DST511 DST511", {explMesPar.Class, explMesPar.Instance, explMesPar.attribute, explMesPar.genericService, explMesPar.specificService, explMesPar.requestStringStructure, explMesPar.resultStringStructure, explMesPar.resultGeneralStatusCode, explMesPar.resultSuccessBit, explMesPar.resultErrorBit}))

        lstUDTconfig.Add("#BEGIN UDT_CONFIG")
        lstUDTconfig.Add("")
        lstUDTconfig.Add("t_ISDU_RW")
        lstUDTconfig.Add("// Field Name, Data Type, Memory Layout, Read/Write, Display Format, Data View Detail")
        lstUDTconfig.Add("Stat, UWORD, 0:0, Read-Write, Native, Short")
        lstUDTconfig.Add("Success, BIT, 0:16, Read-Write, Native, Short")
        lstUDTconfig.Add("Error, BIT, 0:17, Read-Write, Native, Short")
        lstUDTconfig.Add("Write, BIT, 0:18, Read-Write, Native, Short")
        lstUDTconfig.Add("Read, BIT, 0:19, Read-Write, Native, Short")
        lstUDTconfig.Add("Index, UWORD, 1:0, Read-Write, Native, Short")
        lstUDTconfig.Add("SubIndex, UBYTE, 0:3, Read-Write, Native, Short")
        lstUDTconfig.Add("Value, UBYTE, 1:2, Read-Write, Native, Short")

        lstUDTconfig.Add("ISDUReq, STRING32, 2:0, Read-Write, Native, No")
        lstUDTconfig.Add("ISDURes, STRING32, 11:0, Read-Write, Native, No")
        lstUDTconfig.Add("")
        lstUDTconfig.Add("#END")


        lstMEMconfig.Add("#BEGIN MEM_CONFIG")
        lstMEMconfig.Add("ISDU_RW_BUF t_ISDU_RW  0")
        lstMEMconfig.Add("#END")


        Dim sfd As New SaveFileDialog
        sfd.AddExtension = True
        sfd.Filter = "text file | *.txt"
        sfd.FileName = "BrxExplicitMsg_" & vendorNameClean & "_" & deviceNameClean & "_" & tbSubRoutineName.Text.Trim
        If sfd.ShowDialog = DialogResult.OK Then
            Dim lstOut As New List(Of String)
            lstOut.AddRange(lstRungCommands)
            lstOut.AddRange(lstUDTconfig)
            lstOut.AddRange(lstMEMconfig)
            System.IO.File.WriteAllLines(sfd.FileName, lstOut.ToArray)
        End If
    ...
    Public Structure explMsgParam
        ...
        Public genericService As String  '0x25F for specific service
        Public specificService As String 'DST511 if genericService
        Public requestStringStructure As String
        Public resultStringStructure As String
        Public resultGeneralStatusCode As String
        Public resultErrorBit As String
        Public resultSuccessBit As String
    End Structure
End Class